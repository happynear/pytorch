...
THC_API void
THCTensor_(prod)(THCState* state, THCTensor *self, THCTensor *src, int64_t dimension, int keepdim) {
  THCAssertSameGPU(THCTensor_(checkGPU)(state, 2, self, src));
  if (!THC_reduceDim(state, self, src,
                     thrust::identity<real>(),
...
THC_API void
THCTensor_(std)(THCState *state, THCTensor *self_, THCTensor *src, int64_t dimension, int flag, int keepdim)
{
...
THC_API void
THCTensor_(var)(THCState *state, THCTensor *self_, THCTensor *src, int64_t dimension, int flag, int keepdim)
{
  THCAssertSameGPU(THCTensor_(checkGPU)(state, 2, self_, src));
  THLongStorage *dim = THCTensor_(newSizeOf)(state, src);
  THLongStorage_set(dim, dimension, 1);
...
THC_API void
THCTensor_(norm)(THCState *state, THCTensor* self, THCTensor* src, real value, int64_t dimension, int keepdim)
{
  THCAssertSameGPU(THCTensor_(checkGPU)(state, 2, self, src));
